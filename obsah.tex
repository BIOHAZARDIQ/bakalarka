...
V~regresnom testovaní sa z~tohoto dôvodu nespúšťajú všetky testy, 
...
Kontinuálna integrácia je vývoj softvéru, kedy členovia týmu integrujú
...
v~špeciálnom režime spúštať vlastné funkcie, alebo celé testy.
...
dochádzame k~neodpovedajúcim výsledkom z~testovania
...
\begin{table}
  \begin{center}
    ...
    \caption{Prehľad testov dostupných pre najnovšiu verziu produktu MCO 
             a~štatistiky o~dĺzke trvania testov v~sekundách}
...
prerekvizít a~odrekvizíť veľmi podobné. V~produkte SMSCv5 je priemerný 
...
Niektoré testy sa však sú však z~regresnej sady vynechané.
...
Zároveň z~obrázku vypláva to, že sa spustí viac ako dvojnásobok 
...
Distribúcia na základe spoločných prerekvizít využíva faktu, že 
...
Obrovskou výhodou tohoto prístupu je to, že vo výsledku by sme ušetrili 
...
Po rozdelení je dôležíté, aby sme každej časti pridali prerekvizity 
...
najvhodnejší. Problémom tohoto prístupu je však to, že požiadavkom na 
...
Príkladom tohoto problému je prípad, kedy by cluster \emph{cl\_test\_13} 
...
V~prípade, že by ale neexistoval požiadavok na zachovanie funkčnosti, 
...
Rozšírenie je navrhnuté tak, aby bolo  z~jedného stroja spustiť 
...
Tento konfiguračný súbor informácie ako IP adresa systému, na ktorom sa 
...
Pre toto riešenie by však trebalo upraviť parser, ktorý informácie 
...
Táto funkcionalita v~starej verzií plánovača testov chýbala, a~bolo pre 
to ťažké sledovať aktuálny stav testovania. 
...
plánovačov testov vo forme podrocesov, čaká na ukončenie
...
z~tejto podmnožiny regresných testov. Z~tohoto dôvodu by sa výsledky 
...
Medzi jednu z~najdôležitejsích vlastností, ktoré musí každý testovací 
...
riešit stav, v~ktorom sa test mohol ocitnúť. Problémom je situácia, 
kedy nejaký test, ktorý sa spúšťal na viacerých systémoch, skončil na 
týchto rozdielnych systémoch rôznymi výsledkami.
...
v~predchádzajúcej kapitole. Pre riešenie tohoto problému bol zavedený 
...
Problémom tohoto pevného portu bola distribúcia testov, 
...
pristupovali k~rovnakému portu súčastne. Implicitnou hodnotou parametru 
...
Nakoľko firma Acision vyvýja komerčný softvér, pri odovzdávaní zdrojových 
...
takýto súbor. Z~tohoto súboru si vyčíta zoznam clusterov, ktoré sa 
...
Na základe tohoto faktu môžeme čas potrebný na vytvorenie test plánu 
...
Takýmto spôsobom je možné jednoducho kontrolovať počet bežicich 
...
Aby sa zabránilo sutuácií, že by jednotlivé konfiguračné súbory používali 
...
Počítanie tohoto percentuálneho vyjadrenia 
...
nakoľko sa jedná o~kľúčovú chybu. Pre detekovanie tohoto problému pred 
...
Tento čas sa však nepočíta len na základe clusterov, ale do tohoto času 
...
Do tohoto času vykonávania sa však ešte pridajú časy vykonávania 
...
Nevýhodou tohoto prístupu je to, že v~testovacej sade môžeme mať takéto 
...
spustili na jednom testovacom systéme a~prípadné ostatný testovacie 
systémy by ostali nevyužité.

Riešením tohoto problému je použitie optimalizácie, ktorá je schopná 
takúto veľkú skupinu clusterov dozdeliť na menšie. 
...
clusterov do tohoto testovacieho systému by už nemalo byť potrebné.

Algoritmus rozdelovania veľkých skupín clusterov na dve menšie je 
...
rozhodovať sa aj na základe tohoto faktora.
...
s~spomínaných dvoch produktoch každodenne spúšťajú.
...
alebo dokonca aj negatívny vplyv. Optimalizácia 2 funguje na princípe rozdelovania
...
Z~tabuľky vyplýva, že v~prípade nevyužitia optimalizácie sme schopný distribuovať 
iba skupiny clusterov s~rovnakými prerekvizitami. Nakoľko máme takéto skupiny 
iba dve, sme schopný využiť iba dva testovacie systémy. V~prípade využitia
optimalizácie sme schopný rozdelovať skupiny clusterov na menšie, a~využiť 
