...
\projectinfo{
  ...
  %===========================================================================
  %Klicova slova
  keywords.cs={automatické testování softvéru, paralelizace, plánovač, 
               vyhodnocování výsledků, distribuce zátěže, verifikace, 
               validace}, %klicova slova v ceskem jazyce
  keywords.en={automatical software testing, parallelization, scheduler, 
               evaluation of results, load distribution, verification, 
               validation}, %klicova slova v anglickem jazyce
  %Abstract
  abstract.cs={
    ...
    otestování testovaného softwaru danou sadou testů. Vytvořený nástroj 
  ...
  abstract.en={
    ...
    effectively split a~group of test to segments, that will be executed 
    independently at each other. Tests are splitted according to the 
    common characteristics of the enviroment, that need to be prepared 
    for each test. Furthermore, it describes what optimizations are used 
    for splitting of the tests to subgroups. Each subset of the tests is 
    being run on a~standalone testing system, so the time needed for 
  ...
  acknowledgment={
    V~první řadě bych chtěl poděkovat své rodině a~přátelům, 
    ...
    vedoucímu práce, Ing. Františkovi Ščuglíkovi, Ph.D., za jeho cenné 
